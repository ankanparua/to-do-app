...
app.get("/", async (req, res)=>{
    try{
        ...
        if(req.isAuthenticated()){
            const result = await db.query("SELECT * FROM items WHERE user_id = ($1) ORDER BY id ASC",[req.user.id]);
            items = result.rows;
...
app.post("/add", async(req, res)=>{
    ...
    try{
         await db.query("INSERT INTO items (title,date, user_id) VALUES ($1,$2,$3)", [new_task,date_time, req.user.id]);
...
app.post("/edit", async(req, res)=>{
    ...
    try{
        await db.query("UPDATE items SET title = ($1), date = ($2) WHERE id = ($3) AND user_id = ($4)",[item,date_time,id, req.user.id]);
...
app.post("/delete", async(req,res)=>{
    ...
    try{
        await db.query("DELETE FROM items WHERE id = ($1) AND user_id = ($2)",[item, req.user.id]);
